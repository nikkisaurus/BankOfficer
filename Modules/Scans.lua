...
function private:GetMoneyTransactionLabel(scanID, transaction)
    ...
    if private.db.global.settings.dateType == "approx" then
        msg = msg
            and (
                msg
                .. GUILD_BANK_LOG_TIME_PREPEND
                .. date(
                    private.db.global.settings.dateFormat,
                    private:GetTransactionDate(scanID or time(), info.year, info.month, info.day, info.hour)
                )
            )
    else
        msg = msg
